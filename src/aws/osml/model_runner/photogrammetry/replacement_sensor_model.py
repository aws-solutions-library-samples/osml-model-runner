...
class RSMImageDomain:
    ...
    def __init__(
        ...
        :param min_row: int = int of min row
        :param max_row: int = int of max row
        :param min_column: int = int of min column
        :param max_column: int = int of max column
...
class RSMGroundDomain:
    ...
    def __init__(
        ...
        :param ground_domain_form: RSMGroundDomainForm = form of coordinates used by this sensor model
        :param ground_domain_vertices: List[WorldCoordinate] = collection of vertices defining the valid model area
        :param rectangular_coordinate_origin: Optional[WorldCoordinate] = optional origin of the rectangular system
        :param rectangular_coordinate_unit_vectors: Optional[Any] = optional unit vectors to define rectangular system
    ...
    def geodetic_to_ground_domain_coordinate(self, geodetic_coordinate: GeodeticWorldCoordinate) -> WorldCoordinate:
        ...
        :param geodetic_coordinate: GeodeticWorldCoordinate = the WGS-84 longitude, latitude, elevation

        :return: WorldCoordinate = the x, y, z domain coordinate
        """
    ...
    def ground_domain_coordinate_to_geodetic(self, ground_domain_coordinate: WorldCoordinate) -> GeodeticWorldCoordinate:
        ...
        :param ground_domain_coordinate: WorldCoordinate = the x, y, z domain coordinate

        :return: GeodeticWorldCoordinate = the WGS-84 longitude, latitude, elevation coordinate
        """
        if self.ground_domain_form == RSMGroundDomainForm.RECTANGULAR:
            # This shouldn't happen due to the check in the constructor but the type checker is flagging this, so
            # we check again to ensure we have all the values necessary
...
class RSMContext:
    ...
    def __init__(self, ground_domain: RSMGroundDomain, image_domain: RSMImageDomain) -> None:
        ...
        :param ground_domain: RSMGroundDomain = the RSM ground domain
        :param image_domain: RSMImageDomain = the RSM image domain
...
class RSMPolynomial:
    ...
    def __init__(
        ...
        :param max_power_x: int = maximum power of x
        :param max_power_y: int = maximum power of y
        :param max_power_z: int = maximum power of z
        :param coefficients: List[float] = coefficients orders in sequence defined in the NITF RSMPCA TRE
    ...
    def evaluate(self, normalized_world_coordinate: WorldCoordinate) -> float:
        ...
        :param normalized_world_coordinate: WorldCoordinate = the world coordinate
...
class RSMLowOrderPolynomial:
    ...
    def __init__(self, coefficients: List[float]) -> None:
        ...
        :param coefficients: List[float] = the low order polynomial coefficients
    ...
    def evaluate(self, world_coordinate: WorldCoordinate):
        ...
        :param world_coordinate: WorldCoordinate = the world coordinate
...
class RSMSensorModel(SensorModel, ABC):
    ...
    def __init__(self, context: RSMContext) -> None:
        ...
        :param context: RSMContext = contextual information describing the collection environment
...
class RSMPolynomialSensorModel(RSMSensorModel):
    ...
    def __init__(
        ...
        :param context: RSMContext = contextual information describing the collection environment
        :param row_norm_offset: float = offset used to normalize/denormalize image row components
        :param column_norm_offset: float = offset used to normalize/denormalize image column components
        :param x_norm_offset: float = offset used to normalize/denormalize world x components
        :param y_norm_offset: float = offset used to normalize/denormalize world y components
        :param z_norm_offset: float = offset used to normalize/denormalize world z components
        :param row_norm_scale: float = scale used to normalize/denormalize image row components
        :param column_norm_scale: float = scale used to normalize/denormalize image column components
        :param x_norm_scale: float = scale used to normalize/denormalize world x components
        :param y_norm_scale: float = scale used to normalize/denormalize world y components
        :param z_norm_scale: float = scale used to normalize/denormalize world z components
        :param row_numerator_poly: RSMPolynomial = polynomial used as the numerator in row calculations
        :param row_denominator_poly: RSMPolynomial = polynomial used as the denominator in row calculations
        :param column_numerator_poly: RSMPolynomial = polynomial used as the numerator in column calculations
        :param column_denominator_poly: RSMPolynomial = polynomial used as the denominator in column calculations
    ...
    def world_to_image(self, geodetic_coordinate: GeodeticWorldCoordinate) -> ImageCoordinate:
        ...
        :param geodetic_coordinate: GeodeticWorldCoordinate = the world coordinate (longitude, latitude, elevation)

        :return: ImageCoordinate = the resulting image coordinate (x,y)
        """
    ...
    def image_to_world(
        ...
        :param image_coordinate: ImageCoordinate = the image coordinate (x, y)
        :param elevation_model: Optional[ElevationModel] = an optional elevation model used transform the coordinate
        :param options:  Optional[Dict[str, Any]] = optional hints, supports initial_guess and initial_search_distance

        :return: GeodeticWorldCoordinate = the corresponding world coordinate
        """
    ...
    def ground_domain_to_image(self, domain_coordinate: WorldCoordinate) -> ImageCoordinate:
        ...
        :param domain_coordinate: WorldCoordinate = the ground domain coordinate (x, y, z)

        :return: ImageCoordinate = the image coordinate (x, y)
        """
    ...
    def normalize_world_coordinate(self, world_coordinate: WorldCoordinate) -> WorldCoordinate:
        ...
        :param world_coordinate: WorldCoordinate = the world coordinate (longitude, latitude, elevation)

        :return: WorldCoordinate = a world coordinate where each component has been normalized
        """
    ...
    def denormalize_world_coordinate(self, world_coordinate: WorldCoordinate) -> WorldCoordinate:
        ...
        :param world_coordinate: WorldCoordinate = the normalized world coordinate (longitude, latitude, elevation)

        :return: WorldCoordinate = a world coordinate
        """
    ...
    def normalize_image_coordinate(self, image_coordinate: ImageCoordinate) -> ImageCoordinate:
        ...
        :return: ImageCoordinate = the normalized image coordinate (x, y)
        """
    ...
    def denormalize_image_coordinate(self, image_coordinate: ImageCoordinate) -> ImageCoordinate:
        ...
        :param image_coordinate: ImageCoordinate = the normalized image coordinate (x, y)
    ...
    def normalize(value: float, offset: float, scale: float) -> float:
        ...
        :param value: float = the value to be normalized
        :param offset: float = the normalization offset
        :param scale: float = the normalization scale
    ...
    def denormalize(value: float, offset: float, scale: float) -> float:
        ...
        :param value: float = the normalized value
        :param offset: float = the normalization offset
        :param scale: float = the normalization scale

        :return: float = the denormalized value
        """
...
class RSMSectionedPolynomialSensorModel(RSMSensorModel):
    ...
    def __init__(
        ...
        :param context: RSMContext = contextual information describing the collection environment
        :param row_num_image_sections: int = the number of row image sections
        :param column_num_image_sections: int = the number of column image sections
        :param row_section_size: int = the size of row section
        :param column_section_size: int = the size of column section
        :param row_polynomial: RSMLowOrderPolynomial = polynomial used in row calculations
        :param column_polynomial: RSMLowOrderPolynomial = polynomial used in column calculations
        :param section_sensor_models: List[List[SensorModel]] = the list of sensor models
    ...
    def image_to_world(
        ...
        :param image_coordinate: ImageCoordinate = the image coordinate (x, y)
        :param elevation_model: Optional[ElevationModel] = optional elevation model used to transform the coordinate
        :param options:  Optional[Dict[str, Any]] = optional dictionary of hints passed on to the section sensor models

        :return: GeodeticWorldCoordinate = the corresponding world coordinate (longitude, latitude, elevation)
        """
    ...
    def get_section_index(self, image_coordinate: ImageCoordinate) -> Tuple[int, int]:
        ...
        :return: Tuple[int, int] = section index (x, y)
        """
