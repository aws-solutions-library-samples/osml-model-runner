...
def equilateral_triangle(centroid: List[float], size: float) -> List[List[float]]:
    ...
    :param centroid: List[float] = the center of the triangle
    :param size: float = the length of one side

    :return: List[List[float]] = a list of lists representing the triangle len(result) == 3, len(result[0]) == 2
    """
