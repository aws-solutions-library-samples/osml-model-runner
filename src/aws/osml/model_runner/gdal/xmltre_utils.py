...
def get_tre_field_value(tre: ElementTree.Element, field_name: str, type_conversion: Callable[[str], T]) -> T:
    ...
    :param tre: ElementTree.Element = the root element to find the named field in
    :param field_name: str = the name of the field element
    :param type_conversion: Callable[[str], T] = the desired type of the output, must support construction from a string
...
def parse_rpc_coefficients(tre: ElementTree.Element, repeated_name: str) -> List[float]:
    ...
    :param tre: ElementTree.Element = XML document
    :param repeated_name: str = find a specific name in the XML tree

    :return: List[float] = a list of floating point values for the coefficients
    """
